...
import Animated, { interpolateColor, useAnimatedGestureHandler, useAnimatedStyle, useSharedValue, withSpring } from 'react-native-reanimated';
...
const GroupMsgItem = ({ msgData, onSwipeEnabled }) => {
    ...
    return (
        ...
        <FlingGestureHandler direction={dir == 'left' ? Directions.LEFT : Directions.RIGHT}
        ...
        >
            <Animated.View style={[animatedStyle, { width: wp('100') }]}>
                <View style={styles.wholeMsgBox(isCurrentUser)}>
                    ...
                    <Text style={{ color: randomColor }}>{!isCurrentUser ? msgData.sender_name : 'You'}</Text>
                    <Text style={[{ color: 'white', fontSize: 15, textAlign: 'right' }]}>{msgData.text}</Text>
                    <Animated.Text style={[textAnimatedStyle, { fontSize: 10, textAlign: 'right' }]}>{moment(msgData.createdAt).format('hh:mm a ')}</Animated.Text>
                    {/* // {showTick ? */}
                    {msgData.deliverStatus ?

                        <Icons.MaterialIcons name="done" size={15} color={'cyan'} style={{ alignSelf: 'flex-end' }} />
